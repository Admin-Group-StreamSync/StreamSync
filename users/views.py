...
# Llegim el .env cada vegada que es fa una crida (no al importar el mòdul)
# Això evita que cal reiniciar Django si afegeixes el .env després d'arrencar.
def get_api_config():
...
# Noms de plataforma en el mateix ordre que API_BASE_URLS
PLATFORM_NAMES = [n.strip() for n in os.getenv('API_PLATFORM_NAMES', 'CinePlus,StreamHub,PlayMax').split(',') if n.strip()]
...
def map_data(item, base_url):
    """
    Converteix un item de l'API externa al format intern de StreamSync.
    Usa base_url per determinar la plataforma (en lloc del port, que no
    existeix en URLs de Render).
    """
...
def home_page(request):
    ...
    if request.user.is_authenticated:
        try:
            ...
            profile_recommendations = enrich_tmdb_images(enrich(top4))
        except Exception as e:
            print(f"Error filtering preferences: {e}")

    tendencies = enrich_tmdb_images(enrich(all_content[:4]))
    top_rated  = enrich_tmdb_images(enrich(
        sorted(all_content, key=lambda x: float(x.get('rating', 0)), reverse=True)[:4]
    ))
...
def content_detail(request, tipus, content_id):
    ...
    return render(request, 'pagina_contingut.html', {
        'item':           item,
        'tipus':          tipus,
        'ja_guardada':    LlistaPersonal.objects.filter(usuari=request.user, pelicula=movie_db).exists() if request.user.is_authenticated else False,
...
# Legal
def termsofuse_view(request):    return render(request, "legal/terms.html")
def privacy_view(request):       return render(request, "legal/privacy.html")
def cookies_view(request):       return render(request, "legal/cookies.html")
def content_disclaimer_view(request): return render(request, "legal/content_disclaimer.html")


# ─────────────────────────────────────────────
# 7. GESTIÓ DE LLISTES I RESSENYES
# ─────────────────────────────────────────────
@login_required
    ...
    if query:
        ...
        if matches and matches[0][1] > 65:
            ...
            recommendations = [
                ...
                if p['id'] != main_result['id'] and p.get('genre_id') == main_result.get('genre_id')
            ][:5]
...
def dashboard_manager(request, plataforma_nom):
    ...
    try:
        vistes_actuals   = Views.objects.filter(pelicula__in=contingut, visualization_date__gte=fa_30_dies).aggregate(t=Sum('count'))['t'] or 0
        vistes_anteriors = Views.objects.filter(pelicula__in=contingut, visualization_date__gte=fa_60_dies, visualization_date__lt=fa_30_dies).aggregate(t=Sum('count'))['t'] or 0
        ...
        ress_anteriors = Ressenya.objects.filter(pelicula__in=contingut, data_publicacio__gte=fa_60_dies, data_publicacio__lt=fa_30_dies).count()
        ...
        guardats_anteriors = LlistaPersonal.objects.filter(pelicula__in=contingut, data_afegida__gte=fa_60_dies, data_afegida__lt=fa_30_dies).count()
        trend_guardats     = calcular_percentatge(guardats_actuals, guardats_anteriors)

        nota_actual   = Ressenya.objects.filter(pelicula__in=contingut, data_publicacio__gte=fa_30_dies).aggregate(m=Avg('puntuacio'))['m'] or 0
        nota_anterior = Ressenya.objects.filter(pelicula__in=contingut, data_publicacio__gte=fa_60_dies, data_publicacio__lt=fa_30_dies).aggregate(m=Avg('puntuacio'))['m'] or 0
        trend_nota    = round(nota_actual - nota_anterior, 1) if nota_anterior else round(nota_actual, 1)

        usuaris_actuals   = sum(1 for p in Profile.objects.filter(user__date_joined__gte=fa_30_dies) if plataforma_nom in p.plataformes)
        usuaris_anteriors = sum(1 for p in Profile.objects.filter(user__date_joined__gte=fa_60_dies, user__date_joined__lt=fa_30_dies) if plataforma_nom in p.plataformes)
        ...
        top_contingut   = [{
            'id': p.id, 'titol': p.titol, 'imatge': p.imatge, 'any': p.any,
            'tipus': p.tipus, 'rating': p.valoracio, 'vistes_totals': p.vistes_totals,
            'genre_id': '', 'age_rating_id': '', 'genere_nom': '', 'edat_nom': '', 'director_nom': ''
        } for p in top_contingut_db]

        vistes_pelis  = Views.objects.filter(pelicula__in=contingut, pelicula__tipus='movie').aggregate(t=Sum('count'))['t'] or 0
        vistes_series = Views.objects.filter(pelicula__in=contingut, pelicula__tipus='series').aggregate(t=Sum('count'))['t'] or 0
        ...
        visites_per_genere = {}
        for v in Views.objects.filter(pelicula__in=contingut):
            item_api   = next((x for x in totes_api if x['id'] == v.pelicula.id), None)
            nom_genere = mapa_genres.get(str(item_api.get('genre_id')), "Altres") if item_api else "Altres"
        ...
        ratings_api = get_age_ratings_from_api()
        mapa_edats  = {str(r['id']): (r.get('name') or r.get('title') or r.get('description') or "Tots") for r in ratings_api}
        ...
        for i in range(3, -1, -1):
            ...
            evolucio_labels.append(mesos_cat[data_fi.month - 1])
            v = Views.objects.filter(pelicula__in=contingut, visualization_date__gte=data_inici, visualization_date__lt=data_fi).aggregate(t=Sum('count'))['t'] or 0
            u = sum(1 for p in Profile.objects.filter(user__date_joined__gte=data_inici, user__date_joined__lt=data_fi) if plataforma_nom in p.plataformes)
            evolucio_vistes.append(v)
            evolucio_usuaris.append(u)

        for v in Views.objects.filter(pelicula__in=contingut):
            item_api = next((x for x in totes_api if x['id'] == v.pelicula.id), None)
            nom_edat = mapa_edats.get(str(item_api.get('age_rating_id')), "Tots").upper() if item_api else "TOTS"
            if   "18" in nom_edat: edats_dist['18+'] += v.count
            elif "16" in nom_edat: edats_dist['16+'] += v.count
            elif "13" in nom_edat: edats_dist['13+'] += v.count
            elif "7"  in nom_edat: edats_dist['7+']  += v.count
            else:                  edats_dist['Tots'] += v.count
...
# ─────────────────────────────────────────────
# 11. REGISTRE DE VISUALITZACIONS
# ─────────────────────────────────────────────
@login_required
def register_view(request):
    if request.method == "POST":
        try:
            ...
            view_reg, _ = Views.objects.get_or_create(usuari=request.user, pelicula=film, defaults={"count": 0})
            view_reg.count += 1
            view_reg.save()
    ...
    return JsonResponse({"error": "Method not permitted"}, status=405)